     ...
     stages {
         ...
         steps {
              sh '''
                 ...
                 if [[ "$action" == "create" ]]; then cat stacklist | grep "${stack_name}"
                 ...
                 elif [[ "$action" == "delete" ]]; then cat stacklist | grep "${stack_name}"
                  if [[ $? -eq 0 ]]; then echo "Deleting Stack"; aws cloudformation delete-stack --stack-name $stack_name --region us-east-1 ;  else echo "Stack with this name does'nt exist"; fi
                 fi
