     ...
     stages {
         ...
         steps {
              ...
              if [[ "$action" == create ]]
                ...
                aws cloudformation describe-stacks --stack-name $stack_name --region us-east-1 > status.json; cat status.json ;  cat status.json | jq '.Stacks | .[].StackStatus > stackstatus'
                cat stackstatus
