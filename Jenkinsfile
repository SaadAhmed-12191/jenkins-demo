     ...
     stages {
         ...
         steps {
              ...
              if [[ "$action" == create ]]
                ...
                do aws cloudformation describe-stacks --stack-name $stack_name --region us-east-1 > status.json; cat status.json ; var=$(cat status.json | jq '.Stacks | .[].StackStatus'); echo $var
                done    
